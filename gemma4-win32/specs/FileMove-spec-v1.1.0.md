...
Add a separate non-modal `Queue` window opened from the `New / Settings` right-click menu.
...
- Open a context menu with `Settings`, `Status`, `Queue Window`, `Search`, `New`, and `About` by right-clicking `New / Settings`
...
- Open a non-modal Queue window from the `Queue Window` menu entry
